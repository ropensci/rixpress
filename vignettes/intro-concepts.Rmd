...
pipeline by following the next vignette, `vignette("core-functions")`, and
...
spectrum/continuum^[https://doi.org/10.1126/science.1213847],
...
# Generate pipeline.nix and DAG (do not build)
rxp_populate(derivs, project_path = proj, build = FALSE)

# Inspect generated helper files
list.files(file.path(proj, "_rixpress"))
# See the pipeline file
file.path(proj, "pipeline.nix")

# When ready to build (requires Nix & default.nix), run:
# rxp_make(project_path = proj)
```
