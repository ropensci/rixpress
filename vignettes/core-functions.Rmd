...
You can generate and inspect this visualisation before building the pipeline by
adding the `build = FALSE` argument to `rxp_populate()`:

```r
rxp_populate(derivs, build = FALSE)
```
...
and allows you to visualise the graph using `rxp_visnetwork()`, which opens
