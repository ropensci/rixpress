...
`nix-build`. Let’s now drop into our development shell by typping `nix-shell`
...
This template starts with `rxp_r_file()` to load in data, and `rxp_r()` to
define one step. These two derivations are put into a list and then passed to
...
defined called `all-derivations`: this is mostly for internal usage and you
...
don't quite know what the next step should be (maybe there's missing values
you need to handle somehow, or another such cleaning step), but once you know,
you can write the next derivation:
...
with both R and Python derivations, as well compiling a Quarto document.
