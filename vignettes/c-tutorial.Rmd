...
`{rix}`, and `{rixpress}` (copy and paste it into a terminal in the folder where
...
This will drop you into a `Nix` session. Type `R` to start an R session, and then type:
...
To execute a `{rixpress}` pipeline, the environment needs `{rixpress}`
...
CRTL-D or `exit`. Then I can build my development environment using
`nix-build`. Let’s now drop into our development shell by typing `nix-shell`
and launch RStudio (or whatever editor you want to use) and open
`gen-pipeline.R`:
...
`rixpress()`. Let's start by importing data and filtering the data using
`{dplyr}`:
...
went well, you should see the following in your R console:
...
`filtered_mtcars` which is the filtered data, and then an object you didn't
define called `all-derivations`: this is mostly for internal use and you
...
You can now manipulate this object interactively, as usual. Maybe you still
don't quite know what the next step should be (maybe there are missing values
