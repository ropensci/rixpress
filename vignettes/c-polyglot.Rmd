...
For various examples of polyglot pipelines, check out the folder labeled
...
pipelines with derivations that run R or Python code. This vignette explains
...
object to and from Python or R. Python build steps are executed in a standard
Python shell without the need to use `{reticulate}`, so if you're using JSON
to transfer data, there is no need to install `{reticulate}`.

If you want to work on a machine without R nor `{rix}` but with Nix already
installed, we provide a way to bootstrap a complete environment. First, install
cachix and configure the `rstats-on-nix` cache:
...
You only need to do this once per machine you want to use `{rixpress}` on. Many
...
`rxp_quarto()`. Here again, the `additional_files` argument is used to make the
derivation aware of required files to build the document. Here is what the
source of the document looks like:
