...
For a video version of this tutorial, [click here](youtube link to follow).
...
pipeline by following the next vignette `vignette("b-basic-usage")` and then
return to this one; things should be clearer after some hands-on experience!

Also, this package makes heavy use of its sister package `{rix}`, and we highly
recommend you first get familiar with Nix by using `{rix}`. With `{rix}`, you'll
learn how to set up reproducible development environments for data science,
which you can use for interactive data analysis work. Then, if you want to go
one step further in the [reproducibility
...
`mtcars.csv` file is not shared with it.
...
Nix requires dependencies of any derivation to be explicitly listed and also
...
dependency that is not managed by Nix: see the vignette ON CMDSTANR, but these
...
problem which is ensuring complete reproducibility across different environments
