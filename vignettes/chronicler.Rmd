...
`{rixpress}` provides the `rxp_check_chronicles()` function to scan your
pipeline outputs and detect `Nothing` values:
...
This displays status for each chronicle object found:
...
1. **Always run `rxp_check_chronicles()` after building** pipelines that use
   chronicler. Consider adding it to your CI workflow.
