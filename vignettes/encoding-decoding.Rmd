...
vignette: >
  %\VignetteIndexEntry{Encoding, Decoding, and Cross-Language Data Transfer}
...
**Note:** rixpress automatically sets `RETICULATE_AUTOCONFIGURE=0` in
`rxp_py2r()` and `rxp_r2py()` derivations to keep reticulate from modifying
`PYTHONPATH` inside hermetic Nix builds. You can override this by supplying
`env_var` explicitly.
...
- Use `rxp_py2r()` and `rxp_r2py()` if you want to re-use `{reticulate}`'s built-in conversion
