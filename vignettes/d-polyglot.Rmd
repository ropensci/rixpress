...
executing another command. Do it; you only need to do this once per machine you
...
define the execution environment in there:
...
to convert data between R and Python, and only if you can't use JSON to pass
objects to and from Python or R. Python build steps are executed in a standard
Python shell without the need to use `{reticulate}`, so if you're using JSON to
transfer data, there is no need to install `{reticulate}`.

Now that you defined the execution environment of the pipeline, you can run that
script, still from the temporary Nix shell by running `source("gen-env.R")`.
This will generate the required `default.nix`. Then, quit R and the temporary
shell (CTRL-D or `quit()` in R, `exit` in the terminal) and then build the
environment defined by the freshly generated `default.nix` by typing
`nix-build`. This will now build the execution environment of the pipeline. You
can use this environment to work on your project interactively as usual as well.
To learn more, check out [`{rix}`](https://docs.ropensci.org/rix/).
...
learning-specific functions for specific models such as `xgboost`.

## Building a Quarto or R Markdown document

The last pipeline I want to discuss builds a Quarto document using
`rxp_quarto()` (use `rxp_rmd()` for a R Markdown document). Here again, the
...
For an examples that use the Python `xgboost` library to train a machine
learning model, see [this
example](https://github.com/b-rodrigues/rixpress_demos/tree/master/r_py_xgboost).
