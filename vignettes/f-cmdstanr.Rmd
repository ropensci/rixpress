...
`vignette("a-introductory-concepts")` and `vignette("b-core-functions")`.
...
`rxp_r_file()` to bring its content into the pipeline as a character string.
...
6.  **`env_var = c("CMDSTAN" = "${defaultPkgs.cmdstan}/opt/cmdstan")`**: This
    ...
    `cmdstan` would be called something else, say, `cmdstan-env.nix`, then you’d
...
model can then be read back using `rxp_read("model")`, which will internally use
`readRDS()`.

## Summary

Using `{cmdstanr}` with `{rixpress}` involves these key considerations:
1.  **Environment Setup**: Include `cmdstan` in `system_pkgs` and `{cmdstanr}`
    (from Git) in your `{rix}` environment definition.
2.  **Model Code**: Read your `.stan` file into the pipeline as a string using
    `rxp_r_file()`.
3.  **Wrapper Function**: Implement a wrapper function that:
...
4.  **Single Pipeline Step**: Perform model compilation and sampling within the
    *same* `rxp_r()` call using the wrapper.
5.  **`CMDSTAN` Environment Variable**: Set the `CMDSTAN` environment variable
    for the `rxp_r()` step that runs the wrapper, pointing to the Nix store path
    of `cmdstan`.
6.  **Custom Serialization**: Use `{cmdstanr}`'s `$save_object()` method via a
    custom `serialize_function` for robust saving of the fitted model.
