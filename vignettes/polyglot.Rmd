...
list(
  ...
  rxp_py(
    ...
    # first to pandas DF
    name = mtcars_pl_am,
    py_expr = "mtcars_pl.filter(pl.col('am') == 1).to_pandas()"
  ),
...
of them by setting the `additional_files` argument (if derivations need further
files to be available, these can also be specified there. A main difference
between `rxp_py()` and `rxp_r()` is that Python code should be passed as a
string, and not as an expression.
...
list(
  ...
  rxp_py(
    name = mtcars_pl_am,
    py_expr = "mtcars_pl.filter(pl.col('am') == 1)",
