...
This vignette explains how to write a `{rixpress}` pipeline, but doesn't cover
everything you need to know just yet. It also assumes that you've read
`vignette("a-intro-concepts")`. In the next vignette `vignette("c-tutorial")`,
you'll learn how to set up complete pipeline from start to finish.
...
reads a file, regardless of the file type. The path to the file can also be an
url.
...
In the next vignette, we'll learn how to use `{rix}` effectively to provide
...
- as mentioned in the `vignette("a-intro-concepts")`, functions are executed in
  ...
  data on disk, and then you can include it in the pipeline. The only exception
  to this is `rxp_r_file()` which can download a file from an url.
...
- if you have proprietary R packages, you will need to include them in the Nix
  shell. This is more a discussion for `{rix}`, since it is `{rix}` that
  generates the execution environment, so if you require some help to package
  your packages, open an issue [on rix's
  GitHub](https://github.com/ropensci/rix/issues).
