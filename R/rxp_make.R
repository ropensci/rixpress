...
#' @export
rxp_make <- function() {
  system("nix-build --quiet -o_rixpress/result pipeline.nix", intern = TRUE)
}
...
#'   its Nix store.
#'
#' @param store_paths A character vector of Nix store paths to export
#'   (e.g., output from rxp_make()).
#' @param archive_file The file name for the output archive (e.g., "my_archive.nar").
#'
#' @export
export_nix_archive <- function(store_paths, archive_file) {
